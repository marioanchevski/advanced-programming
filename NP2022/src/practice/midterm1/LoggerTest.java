...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = (logs) -> logs.stream().
                filter(log -> log.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor = (logs) -> logs.stream().
                filter(log -> log.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));
