package auditory_exercises.aud05.weightcontainer;
...
class Container<T extends Weightable> {
    ...
    public List<T> lighterThan(T element) {
        return elements.stream().filter(i -> i.compareTo(element) < 0)
                .collect(Collectors.toList());
    }

    public List<T> between(T a, T b) {
        return elements.stream().filter(i -> i.compareTo(a) > 0 && i.compareTo(b) < 0)
                .collect(Collectors.toList());
    }
    ...
    public int compare(Container<? extends Weightable> container2) {
        return Double.compare(getContainerWeight(), container2.getContainerWeight());
    }
}
